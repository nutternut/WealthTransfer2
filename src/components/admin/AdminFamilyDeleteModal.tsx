...
import type { AdminFamilyRow } from "@/lib/admin-db";

type AdminFamilyDeleteModalProps = {
  open: boolean;
...
export function AdminFamilyDeleteModal({
  open,
  family,
  busy = false,
  onClose,
  onConfirm,
}: AdminFamilyDeleteModalProps) {
  const titleId = useId();
        ...
        <div className="relative w-full max-w-sm overflow-hidden rounded-2xl border border-slate-100 bg-white text-left shadow-xl">
          ...
          <div className="space-y-5 p-6">
            ...
            <p className="text-xs text-slate-400">
              บัญชี {family.ownerUsername} ยังอยู่
              สมาชิกและทรัพย์สินของเจ้าของไม่ถูกลบ
            </p>
